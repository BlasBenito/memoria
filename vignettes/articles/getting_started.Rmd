...
The `prepareLaggedData()` function organizes data into the lag structure required by the model. Each sample of the response is aligned with antecedent values of itself and the drivers. In this case we use lags 1 to 10, which correspond with 1000 to 10000 years.

```{r prepare-lagged}
quercus_lags <- prepareLaggedData(
  input.data = df,
  response = "quercus",
  drivers = c("rain", "temp"),
  time = "time",
...
```{r compute-memory-autocor, fig.width = 5, fig.height = 4}
