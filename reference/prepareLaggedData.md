...
Takes a multivariate time series, where at least one variable is meant
to be used as a response while the others are meant to be used as
predictors in a model, and organizes it in time lags, generating one new
column per lag and variable in the model.
...
- oldest.sample:
  ...
  `input.data` represents a palaeoecological dataset. Default behavior
  is "first".
