...
#'
#'
#' @param memory.pattern either a list resulting from \code{\link{computeMemory}}, or a dataframe with memory patterns of several taxa generated by \code{\link{experimentToTable}}. Default: \code{NULL}.
...
#'   Required when input is from \code{\link{experimentToTable}}. Default: \code{NULL}.
#' @param sampling.subset only relevant when \code{memory.pattern} is the result of \code{runExperiment}. Character string with the name of the column of the list with the simulation outcomes. Default: \code{NULL}.
...
extractMemoryFeatures <- function(
  ...
  if (!is.computeMemory.object) {
    ...
    if (!endogenous.component %in% available_vars) {
      stop(
        "Endogenous component '", endogenous.component,
