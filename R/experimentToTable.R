...
experimentToTable <- function(
  ...
  for (current.row in 1:list.rows) {
    for (current.column in 1:list.columns) {
      ...
      if (list.columns > 1) {
        ...
        temp.data <- experiment.output$output[[
          current.row,
          current.column
        ]]$memory
        ...
        if (R2 == TRUE) {
          temp.data$name = paste(
            ...
            "; R2 ",
            round(
              mean(experiment.output$output[[current.row, current.column]]$R2),
              2
            ),
            "sd ",
            round(
              sd(experiment.output$output[[current.row, current.column]]$R2),
              2
            ),
            sep = ""
          )
        } else {
          temp.data$name <- experiment.output$names[[
            current.row,
            current.column
          ]]
        }
        ...
        temp.data$R2mean <- mean(
          experiment.output$output[[current.row, current.column]]$R2
        )
        temp.data$R2sd <- sd(
          experiment.output$output[[current.row, current.column]]$R2
        )
        ...
        if (R2 == TRUE) {
          temp.data$name <- paste(
            ...
            "; R2 ",
            round(mean(experiment.output$output[[current.row]]$R2), 2),
            "sd ",
            round(sd(experiment.output$output[[current.row]]$R2), 2),
            sep = ""
          )
        } else {
        ...
        #getting average multicollinearity
        temp.data$VIFmean <- mean(
          experiment.output$output[[current.row]]$multicollinearity$vif
        )
        temp.data$VIFsd <- sd(
          experiment.output$output[[current.row]]$multicollinearity$vif
        )
      ...
      temp.data <- cbind(
        ...
        as.data.frame(lapply(
          parameters.file[
            parameters.file$label == species.names[current.row],
          ],
          rep,
          nrow(temp.data)
        ))
  ...
  simulation.df$name <- gsub(
    pattern = "autocorrelation.length.A",
    replacement = "Aac",
    x = simulation.df$name
  )
  simulation.df$name <- gsub(
    pattern = "autocorrelation.length.B",
    replacement = "Bac",
    x = simulation.df$name
  )
